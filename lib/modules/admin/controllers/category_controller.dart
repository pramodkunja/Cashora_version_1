...
/// Admin management of expense categories. Mirrors [DepartmentController] so the
/// Categories screen behaves identically to the Departments screen.
class CategoryController extends GetxController {
  ...
  Future<void> fetchCategories() async {
    ...
    try {
      ...
      errorMessage.value = _extractError(e, 'Failed to load categories');
    } finally {
  ...
  Future<void> createCategory() async {
    ...
    try {
      ...
      Get.snackbar('Success', 'Category "$name" created',
          snackPosition: SnackPosition.BOTTOM,
          backgroundColor: AppColors.successGreen,
          colorText: Colors.white,
          margin: const EdgeInsets.all(16));
      ...
      Get.back(); // loader
      Get.snackbar('Error', _extractError(e, 'Failed to create category'),
          backgroundColor: Colors.red.shade100, colorText: Colors.red);
    }
  }
  ...
  void showEditDialog(Map<String, dynamic> category) {
    ...
    codeController.text = category['code']?.toString() ?? '';
    Get.dialog(_buildFormDialog(isEdit: true, categoryId: category['id']));
  }
  ...
  Future<void> updateCategory(int id) async {
    ...
    try {
      ...
      Get.back(); // loader
      Get.snackbar('Error', _extractError(e, 'Failed to update category'),
          backgroundColor: Colors.red.shade100, colorText: Colors.red);
    }
  }
  ...
  Future<void> deleteCategory(int id, String name) async {
    final confirmed = await Get.dialog<bool>(
      AlertDialog(
        ...
        content: Text(
            'Deactivate "$name"? It will no longer be selectable for new requests.'),
        actions: [
          TextButton(
    ...
    try {
      ...
      Get.back();
      Get.snackbar('Error', _extractError(e, 'Failed to deactivate category'),
          backgroundColor: Colors.red.shade100, colorText: Colors.red);
    }
  }
  ...
  Future<void> reactivateCategory(int id, String name) async {
    final confirmed = await Get.dialog<bool>(
      AlertDialog(
        title: const Text('Reactivate Category'),
        content: Text('Reactivate "$name"? It will be selectable again.'),
        actions: [
          TextButton(
              onPressed: () => Get.back(result: false),
              child: const Text('Cancel')),
    ...
    try {
      ...
      await _repository.updateCategory(id, isActive: true);
      Get.back();
      ...
      Get.back();
      Get.snackbar('Error', _extractError(e, 'Failed to reactivate category'),
          backgroundColor: Colors.red.shade100, colorText: Colors.red);
    }
  }
  ...
  Widget _buildFormDialog({required bool isEdit, int? categoryId}) {
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Title
                    Text(
                      isEdit ? 'Update category' : 'Create a category',
                    ...
                    Text(
                      isEdit
                          ? 'Update the name or code for this category.'
                          : 'Group expense requests under a category.',
                      style: GoogleFonts.inter(
                        fontSize: 13.sp,
                        color: ink500,
                        height: 1.45,
