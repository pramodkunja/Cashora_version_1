...
/// One row in the category list — gradient icon disc, name + code +
/// optional INACTIVE badge, three-dot actions menu (edit / deactivate
/// / reactivate). Mirrors `DepartmentCard` so the two screens feel
/// identical.
...
/// Three-dot kebab on each category row.
class CategoryCardActionsMenu extends StatelessWidget {
  final Map<String, dynamic> category;
  final int id;
  final String name;
  final bool isActive;
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<String>(
      ...
      onSelected: (v) {
        if (v == 'edit') controller.showEditDialog(category);
        if (v == 'delete') controller.deleteCategory(id, name);
        if (v == 'reactivate') controller.reactivateCategory(id, name);
      },
      itemBuilder: (_) => [
        PopupMenuItem(
          value: 'edit',
